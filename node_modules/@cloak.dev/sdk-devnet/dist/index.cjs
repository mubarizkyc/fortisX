...
__export(utxo_exports, {
  ...
  deriveUtxoKeypairFromSpendKey: () => deriveUtxoKeypairFromSpendKey,
  deserializeUtxo: () => deserializeUtxo,
  generateUtxoKeypair: () => generateUtxoKeypair,
  hexToBigint: () => hexToBigint2,
  ...
  sumUtxoAmounts: () => sumUtxoAmounts,
  utxoEquals: () => utxoEquals
});
async function getPoseidon2() {
  if (!poseidonInstance) {
    poseidonInstance = await (0, import_circomlibjs2.buildPoseidon)();
  }
...
function randomFieldElement() {
  ...
  if (typeof crypto !== "undefined" && crypto.getRandomValues) {
    ...
    const nodeBytes = randomBytes2(32);
    for (let i = 0; i < 32; i++) {
      bytes[i] = nodeBytes[i];
    }
  }
...
async function generateUtxoKeypair() {
  ...
  const publicKeyHash = poseidon3([privateKey, BigInt(0)]);
  const publicKey = poseidon3.F.toObject(publicKeyHash);
...
async function deriveUtxoKeypairFromSpendKey(skSpend) {
  ...
  const hash = (0, import_blake32.blake3)(preimage);
  let value = BigInt(0);
  for (let i = 0; i < 32; i++) {
...
async function derivePublicKey(privateKey) {
  const poseidon3 = await getPoseidon2();
  const hash = poseidon3([privateKey, BigInt(0)]);
...
async function createZeroUtxo(mintAddress = NATIVE_SOL_MINT, salt) {
  ...
  const publicKeyHash = poseidon3([effectiveSalt, BigInt(0)]);
  const publicKey = poseidon3.F.toObject(publicKeyHash);
...
async function computeNullifier2(utxo) {
  ...
  const hash = poseidon3([commitment, pathIndex, signature]);
  return poseidon3.F.toObject(hash);
}
...
__export(index_exports, {
  ...
  DEVNET_MOCK_USDC_MINT: () => DEVNET_MOCK_USDC_MINT,
  ...
  calculateFeeBigint: () => calculateFeeBigint,
  ...
  deriveUserCompliancePublicKey: () => deriveUserCompliancePublicKey,
  deriveUserComplianceScalar: () => deriveUserComplianceScalar,
  deriveUtxoKeypairFromSpendKey: () => deriveUtxoKeypairFromSpendKey,
  deriveViewKey: () => deriveViewKey,
  deriveViewingKeyFromNk: () => deriveViewingKeyFromNk,
  deriveViewingKeyFromSpendKey: () => deriveViewingKeyFromSpendKey,
...
var RelayService = class {
  ...
  computeViewingKeyIdentifier(userPubkey, viewingKeyHex) {
    ...
    const hash = (0, import_sha2.sha256)(preimage);
...
function createLogger(module2) {
  const log = (level, message, kvPairs) => {
    ...
    if (useColors) {
      ...
      line = `${timestamp} ${color}${levelPadded}${reset} ${module2}: ${message}`;
    } else {
      line = `${timestamp} ${levelPadded} ${module2}: ${message}`;
    }
...
var CLOAK_PROGRAM_ID = new import_web35.PublicKey("Zc1kHfp4rajSMeASFDwFFgkHRjv7dFQuLheJoQus27h");
var DEVNET_MOCK_USDC_MINT = new import_web35.PublicKey("61ro7AExqfk4dZYoCyRzTahahCC2TdUUZ4M5epMPunJf");
...
function expandSpendKey(skSpend) {
  ...
  const mkPreimage = (domain) => {
    ...
    return (0, import_blake33.blake3)(p);
  };
  return {
...
function deriveViewingKeyFromNk(nk) {
  ...
  const derived = (0, import_blake33.blake3)(preimage);
...
function deriveDiversifier(nk, commitmentHex, outputIndex) {
  ...
  const hash = (0, import_blake33.blake3)(preimage);
...
function bytesToFieldElement(bytes) {
  return bytesToBigint2(bytes) % BN254_MODULUS;
}
...
function computeViewingKeyIdentifier(userPubkey, nkHex) {
  ...
  return bytesToHex2((0, import_sha22.sha256)(preimage));
}
...
var import_circomlibjs3 = require("circomlibjs");
var MERKLE_TREE_HEIGHT2 = 32;
var poseidonInstance2 = null;
async function getPoseidon3() {
  if (!poseidonInstance2) {
    poseidonInstance2 = await (0, import_circomlibjs3.buildPoseidon)();
  }
  return poseidonInstance2;
}
var ZERO_VALUES = null;
async function getZeroValues() {
  if (ZERO_VALUES) return ZERO_VALUES;
  const poseidon3 = await getPoseidon3();
  const zeros = [];
  zeros[0] = BigInt(0);
...
var poseidon2 = null;
async function getPoseidon4() {
  if (!poseidon2) {
    poseidon2 = await (0, import_circomlibjs4.buildPoseidon)();
  }
  return poseidon2;
}
...
async function computeChainNoteHash(publicAmount, extDataHash, noteTimestamp, noteCommitment) {
  const poseidonHasher = await getPoseidon4();
...
async function ensureViewingKeyRegistered(options, onProgress, fallbackNk) {
  ...
  if (options.signMessage) {
    ...
    const messageBytes = new TextEncoder().encode(challengeMessage);
    const sig = import_tweetnacl4.default.sign.detached(messageBytes, options.depositorKeypair.secretKey);
    signatureBase64 = Buffer.from(sig).toString("base64");
  } else {
    throw new Error(
...
function proofToBytesLocal(proof) {
  ...
  result.set(bigintToBytes322(pi_a_x), offset);
  offset += 32;
  result.set(bigintToBytes322(pi_a_y_neg), offset);
  offset += 32;
  ...
  result.set(bigintToBytes322(pi_b_x_c1), offset);
  offset += 32;
  result.set(bigintToBytes322(pi_b_x_c0), offset);
  offset += 32;
  result.set(bigintToBytes322(pi_b_y_c1), offset);
  offset += 32;
  result.set(bigintToBytes322(pi_b_y_c0), offset);
  offset += 32;
  const pi_c_x = BigInt(proof.pi_c[0]);
  const pi_c_y = BigInt(proof.pi_c[1]);
  result.set(bigintToBytes322(pi_c_x), offset);
  offset += 32;
  result.set(bigintToBytes322(pi_c_y), offset);
  return result;
}
function buildPublicInputsBytesFromSignals(publicSignals) {
  ...
  let offset = 0;
  result.set(bigintToBytes322(BigInt(publicSignals[0])), offset);
  offset += 32;
  let amount = BigInt(publicSignals[1]);
  const FIELD_MODULUS2 = BigInt("21888242871839275222246405745257275088548364400416034343698204186575808495617");
  const HALF_FIELD = FIELD_MODULUS2 / BigInt(2);
  if (amount > HALF_FIELD) {
    amount = amount - FIELD_MODULUS2;
  }
  const amountBytes = new Uint8Array(8);
  ...
  offset += 8;
  result.set(bigintToBytes322(BigInt(publicSignals[2])), offset);
  offset += 32;
  result.set(bigintToBytes322(BigInt(publicSignals[3])), offset);
  offset += 32;
  result.set(bigintToBytes322(BigInt(publicSignals[4])), offset);
  offset += 32;
  result.set(bigintToBytes322(BigInt(publicSignals[5])), offset);
  offset += 32;
  result.set(bigintToBytes322(BigInt(publicSignals[6])), offset);
  offset += 32;
  result.set(bigintToBytes322(BigInt(publicSignals[7])), offset);
  offset += 32;
  result.set(bigintToBytes322(BigInt(publicSignals[8])), offset);
  offset += 32;
  return result;
}
...
async function submitTransactionDirect(connection, programId, depositor, proofBytes, publicInputsBytes, nullifiers, mint, recipient, riskOracleQueue, riskQuoteUrl, getRiskQuoteInstruction, onProgress, addressLookupTableAccounts, rangeApiKey, encryptedNoteBytes, relayUrl, altAddresses) {
  ...
  let splAccounts;
  if (isSplPool) {
    const poolVaultAta = (0, import_spl_token.getAssociatedTokenAddressSync)(
      mint,
      pdas.vaultAuthority,
      true,
    ...
    if (publicAmount > BigInt(0)) {
      splAccounts.payerAta = (0, import_spl_token.getAssociatedTokenAddressSync)(
        mint,
        depositor.publicKey,
        false,
        import_spl_token.TOKEN_PROGRAM_ID
...
async function transact(params, options) {
  ...
  while (submissionAttempts <= maxRootRetries) {
    ...
    const FIELD_MODULUS2 = BigInt("21888242871839275222246405745257275088548364400416034343698204186575808495617");
    if (chainNoteHash >= FIELD_MODULUS2) {
      throw new Error(`chainNoteHash out of range: ${chainNoteHash.toString()} >= ${FIELD_MODULUS2.toString()}`);
    }
    ...
    if (isDeposit) {
      ...
      try {
        const nullifier0Bytes = bigintToBytes322(inputNullifiers[0]);
        const nullifier1Bytes = bigintToBytes322(inputNullifiers[1]);
...
async function swapUtxo(params, options) {
  ...
  for (let attempt = 0; attempt <= maxRootRetries; attempt++) {
    ...
    const poseidonHasherSwap = await getPoseidon4();
...
var UtxoWallet = class _UtxoWallet {
  ...
  async createOutputUtxo(amount, _recipient) {
    const { randomFieldElement: randomFieldElement2, derivePublicKey: derivePublicKey2 } = await Promise.resolve().then(() => (init_utxo(), utxo_exports));
