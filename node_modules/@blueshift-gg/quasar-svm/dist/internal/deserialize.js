...
function skipAccountFields(data, o) {
    o += 32; // owner
    o += 8; // lamports
...
export function deserializeResult(data) {
    ...
    for (let i = 0; i < numAccts; i++) {
        const address = new Uint8Array(data.subarray(o, o + 32));
        o += 32;
        const owner = new Uint8Array(data.subarray(o, o + 32));
        o += 32;
    ...
    const numPreBalances = data.readUInt32LE(o);
    o += 4;
    const preBalances = [];
    ...
    const numPostBalances = data.readUInt32LE(o);
    o += 4;
    const postBalances = [];
    ...
    // Token balances (pre)
    const numPreTokenBalances = data.readUInt32LE(o);
    o += 4;
    const preTokenBalances = [];
    for (let i = 0; i < numPreTokenBalances; i++) {
        const accountIndex = data.readUInt32LE(o);
        o += 4;
        const mintLen = data.readUInt32LE(o);
        o += 4;
        const mint = data.subarray(o, o + mintLen).toString("utf8");
        o += mintLen;
        const hasOwner = data[o++] !== 0;
        const owner = hasOwner ? (() => {
            const ownerLen = data.readUInt32LE(o);
            o += 4;
            const ownerStr = data.subarray(o, o + ownerLen).toString("utf8");
            o += ownerLen;
            return ownerStr;
        })() : null;
        const decimals = data[o++];
        const amountLen = data.readUInt32LE(o);
        o += 4;
        const amount = data.subarray(o, o + amountLen).toString("utf8");
        o += amountLen;
    ...
    // Token balances (post)
    const numPostTokenBalances = data.readUInt32LE(o);
    o += 4;
    ...
    const numInstructions = data.readUInt32LE(o);
    o += 4;
    const instructions = [];
    for (let i = 0; i < numInstructions; i++) {
        const stackDepth = data[o++];
        // Read full instruction data
        const programId = new Uint8Array(data.subarray(o, o + 32));
        o += 32;
        const numAccounts = data.readUInt32LE(o);
        o += 4;
        const accounts = [];
        for (let j = 0; j < numAccounts; j++) {
            const pubkey = new Uint8Array(data.subarray(o, o + 32));
            o += 32;
            const isSigner = data[o++] !== 0;
            const isWritable = data[o++] !== 0;
            accounts.push({ pubkey, isSigner, isWritable });
        }
        const dataLen = data.readUInt32LE(o);
        o += 4;
        const ixData = new Uint8Array(data.subarray(o, o + dataLen));
        o += dataLen;
        // Read compute units and result
        const computeUnitsConsumed = data.readBigUInt64LE(o);
        o += 8;
        const result = data.readBigUInt64LE(o);
        o += 8;
        instructions.push({
            stackDepth,
    ...
    const status = rawStatus === 0
        ? { ok: true }
        : { ok: false, error: programErrorFromStatus(rawStatus, errorMessage) };
    return {
        status,
        computeUnits,
        executionTimeUs,
